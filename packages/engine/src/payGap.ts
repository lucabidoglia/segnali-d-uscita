/**
 * Indicatori della Direttiva (UE) 2023/970 sulla trasparenza retributiva, art. 9 par. 1 lett. a–g.
 *
 * Scelte di metodo (da confermare con il decreto italiano di recepimento):
 * - il "livello retributivo" è la retribuzione lorda ORARIA = (base + variabile) ÷ ore retribuite (art. 3);
...
export interface CategoryGap {
  ...
  /** Divario scomposto: retribuzione di base e componenti variabili (art. 9, lett. g). */
  gapBaseMean: number | null;
  gapVariableMean: number | null;
  /** Candidata alla valutazione congiunta (art. 10) se |divario medio| ≥ 5%. */
...
const hourlyBase = (w: Worker): number => w.basePay / w.hoursPaid;
...
export function payGapReport(workers: readonly Worker[], opts: PayGapOptions = {}): PayGapReport {
  ...
  const sorted = [...inScope].sort((a, b) => hourlyTotal(a) - hourlyTotal(b));
  const quartiles: Quartile[] = ([1, 2, 3, 4] as const).map(q => {
    ...
    return {
      ...
      minHourly: grp.length ? hourlyTotal(grp[0]!) : NaN, maxHourly: grp.length ? hourlyTotal(grp.at(-1)!) : NaN
    };
  });
  ...
  const categories: CategoryGap[] = [...new Set(inScope.map(w => w.category))].sort().map(category => {
    ...
    const gapMean = published ? gapOf(cg, hourlyTotal, mean) : null;
    return {
      category, n: { F: cg.F.length, M: cg.M.length }, published,
      meanHourly: published ? { F: mean(cg.F.map(hourlyTotal)), M: mean(cg.M.map(hourlyTotal)) } : null,
      gapMean,
      gapMedian: published ? gapOf(cg, hourlyTotal, median) : null,
      gapBaseMean: published ? gapOf(cg, hourlyBase, mean) : null,
  ...
  return {
    ...
    gapMean: gapOf(g, hourlyTotal, mean),
    gapMedian: gapOf(g, hourlyTotal, median),
...
/**
 * Obbligo di comunicazione del divario per dimensione (art. 9 par. 2–4).
 * Il recepimento nazionale può anticipare o estendere gli obblighi: verificare il decreto italiano.
 */
export function reportingObligation(headcount: number): ReportingObligation {
  if (headcount >= 250) return { required: true, frequency: 'annuale', firstDeadline: '2027-06-07', note: 'Dati dell\'anno solare precedente, ogni anno.' };
  if (headcount >= 150) return { required: true, frequency: 'triennale', firstDeadline: '2027-06-07', note: 'Ogni tre anni.' };
  if (headcount >= 100) return { required: true, frequency: 'triennale', firstDeadline: '2031-06-07', note: 'Ogni tre anni.' };
  return { required: false, frequency: null, firstDeadline: null, note: 'Sotto i 100 lavoratori la comunicazione è volontaria, salvo diverso obbligo nazionale.' };
}
