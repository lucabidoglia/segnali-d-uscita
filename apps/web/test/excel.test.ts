...
describe("import Excel", () => {
  it("l'esempio con 180 persone inventate si rilegge senza errori e produce il report", async () => {
    const { workers, issues } = await parseWorkbook(ab(await templateWorkbook(demo)));
  ...
  it("accetta numeri scritti all'italiana e segnala celle sbagliate con il numero di riga", async () => {
    const buf = await templateWorkbook([
      { id: "A1", gender: "f", category: "X", funzione: "X", fte: "0,5", hoursPaid: "860", basePay: "12.345,67", variablePay: "" },
    ...
    expect(workers[0]).toMatchObject({ gender: "F", fte: 0.5, hoursPaid: 860, basePay: 12345.67, variablePay: 0 });
